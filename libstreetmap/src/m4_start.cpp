...
//std::unordered_map<pairInt, bool, hash> best_path;
...
//Seg_array tour_construction(IntersectionIndex i,
//        double q, double turn_penalty,
//        const std::unordered_map<IntersectionIndex, bool>& legal_points,
//        IntersectionIndex& j,
//        double& time_cost);
//
...
//                double local_cost;
//                current_segment_path.push_back(tour_construction(current_point, dist(generator), turn_penalty, ant_points, next_point, local_cost));
//                
...
//}
//
//IntersectionIndex rand_prop_rule(IntersectionIndex i, double turn_penalty, const std::unordered_map<IntersectionIndex, bool>& legal_points) {
...
//        double current_prob = pow(getPheromone(i, it->first), ALPHA) * pow(compute_path_travel_time(find_path_between_intersections(i, it->first, turn_penalty), turn_penalty), BETA);
...
//
//// ant travels path i to the intersection j (returned in this func)
//Seg_array tour_construction(IntersectionIndex i, double q, double turn_penalty, const std::unordered_map<IntersectionIndex, bool>& legal_points, IntersectionIndex& j, double& time_cost) {
...
//        
//        for (auto it = legal_points.begin(); it != legal_points.end(); ++it) {
//            Seg_array current_path = find_path_between_intersections(i, it->first, turn_penalty);
//            double current_time = compute_path_travel_time(current_path, turn_penalty);
//            double current_cost = getPheromone(i, it->first) * pow((1 / current_time), BETA);
//            
...
//    
//    else {
//        j = rand_prop_rule(i, turn_penalty, legal_points);
//        path = find_path_between_intersections(i, j, turn_penalty);
//        time_cost = compute_path_travel_time(path, turn_penalty);
//    }
//    
//    localUpdate(i, j);
//    return path;
